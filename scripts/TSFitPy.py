...
def chi_square_broad(param, obs_name, temp_directory, spectrum_count, mask_file, segment_file, depart_bin_file_list,
                     ...
                     vturb, macro, fwhm, rot, abund_name, abund_low, abund_high, ldelta, lmin, lmax):
    # teff = param[0]
    # logg = param[1]
    # met = param[2]
...
def chi_square_broad_lbl(param, obs_name, temp_directory, spectrum_count, depart_bin_file_list, depart_aux_file_list,
                         model_atom_file_list, atmosphere_type, nlte_flag, doppler, teff, logg, met, fit_microturb,
                         macro, fwhm, rot, abund_name, abund_low, abund_high, ldelta, lmin, lmax):
    abund = param[0]
    doppler = doppler + param[-1]
...
def chi_square_broad_met_lbl(param, obs_name, temp_directory, spectrum_count, depart_bin_file_list,
                             depart_aux_file_list, model_atom_file_list, atmosphere_type, nlte_flag, doppler, teff,
                             logg, fit_microturb, macro, fwhm, rot, ldelta, lmin, lmax):
    met = param[0]
    doppler = doppler + param[-1]
...
if atmosphere_type == "1D":
    model_atmosphere_grid_path = "/mnt/beegfs/gemini/scratch/storm/TSFitPy2/input_files/model_atmospheres/1D/"
    model_atmosphere_list = model_atmosphere_grid_path + "model_atmosphere_list.txt"
elif atmosphere_type == "3D":
    model_atmosphere_grid_path = "/mnt/beegfs/gemini/scratch/storm/TSFitPy2/input_files/model_atmospheres/3D/"
    model_atmosphere_list = model_atmosphere_grid_path + "model_atmosphere_list.txt"
...
if fitting_mode == "all" and (element[0] == "Fe" or element[0] == "fe"):
    ...
    print(
        "#specname        wave_center  wave_start  wave_end  [Fe/H]    Microturb     Doppler Shift (add to RV)    chi_squared",
        file=f)
elif fitting_mode == "lbl":
    print(
        "#specname        wave_center  wave_start  wave_end  [{}/Fe]   Microturb     Doppler Shift (add to RV)    chi_squared".format(
            element[0]), file=f)
...
if fitting_mode == "all":
    ...
    create_window_linelist(segment_file, line_list_path_orig, line_list_path_trimmed, include_molecules, trimmed_start,
                           trimmed_end)
else:
...
for i in range(specname_fitlist.size):
    ...
    if fitting_mode == "all":
        ...
        if element[0] == "Fe" or element[0] == "fe":
            ...
            if nlte_flag == "True":
                met_auxdata, abund_values_auxdata = np.loadtxt("{}{}".format(departure_file_path, depart_aux_file[0]),
                                                               usecols=(3, 7), unpack=True)
                ...
                abund_low = -99.
                abund_high = 99
            res = minimize(chi_square_broad, param0, args=(
                np.str(specname), temp_directory, i, linemask_file, segment_file, depart_bin_file, depart_aux_file,
                model_atom_file, atmosphere_type, nlte_flag, rv, teff, logg, met, vturb, macroturb, fwhm, rot, element,
                abund_low, abund_high, ldelta, lmin, lmax), method='Nelder-Mead',
                           options={'maxiter': ndimen * 50, 'disp': True, 'initial_simplex': initial_guess,
                                    'xatol': 0.05, 'fatol': 0.05})
            print(res.x)
            ...
            print("{} {} {} {}".format(specname.replace("../input_files/observed_spectra/", ""), res.x[0], res.x[1],
                                       res.fun), file=f)
        ...
        if element[0] == "Fe" or element[0] == "fe":
            ...
            for j in range(len(line_begins_sorted)):
                ...
                res = minimize(chi_square_broad_met_lbl, param0, args=(
                    np.str(specname), temp_directory, i, depart_bin_file, depart_aux_file, model_atom_file,
                    atmosphere_type,
                    nlte_flag, rv, teff, logg, fit_microturb, macroturb, fwhm, rot, ldelta, line_begins_sorted[j] - 5.,
                    line_ends_sorted[j] + 5.), method='Nelder-Mead',
                               options={'maxiter': ndimen * 50, 'disp': True, 'initial_simplex': initial_guess,
                                        'xatol': 0.05, 'fatol': 0.05})
            ...
            for j in range(len(line_begins_sorted)):
                ...
                res = minimize(chi_square_broad_lbl, param0, args=(
                    np.str(specname), temp_directory, i, depart_bin_file, depart_aux_file, model_atom_file,
                    atmosphere_type,
                    nlte_flag, rv, teff, logg, met, fit_microturb, macroturb, fwhm, rot, element, abund_low, abund_high,
                    ldelta, line_begins_sorted[j] - 5., line_ends_sorted[j] + 5.), method='Nelder-Mead',
                               options={'maxiter': ndimen * 50, 'disp': True, 'initial_simplex': initial_guess,
                                        'xatol': 0.05, 'fatol': 0.05})
